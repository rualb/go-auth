...
func TestAuthSigninEchoHandler(t *testing.T) {
	...
	/*
		const testOnlyID = "test-only-id"
		const testOnlyPhone = "+999999999999"
	...
	*/
	...
	}
	{
		...
		req := httptest.NewRequest(http.MethodGet, consts.PathAuthSigninPhoneNumberAPI, nil)
		rec := httptest.NewRecorder()
		c := e.NewContext(req, rec)
		if err := handler(c); err != nil {
		...
		tests := []struct {
		...
		}{
			{
				...
				expected: statusResponse{IsSuccess: true},
				body:     &reqBody{PhoneNumber: testOnlyUserPhone, Password: testOnlyUserPw},
			},
			{
				title:    "test 2",
				cookie:   "", // "_csrf=" + url.QueryEscape(csrfToken),
				expected: statusResponse{IsSuccess: false},
				body:     &reqBody{PhoneNumber: testOnlyUserPhone, Password: testOnlyUserPw + "1"},
			},
			{
				title:    "test 3",
				cookie:   "", // "_csrf=" + url.QueryEscape(csrfToken),
				expected: statusResponse{IsSuccess: false},
				body:     &reqBody{PhoneNumber: testOnlyUserPhone + "1", Password: testOnlyUserPw},
			},
			{
				title:    "test 1-2",
				cookie:   "", // "_csrf=" + url.QueryEscape(csrfToken),
		...
		for _, tc := range tests {
			t.Run(tc.title, func(t *testing.T) {
				...
				req := httptest.NewRequest(http.MethodPost, consts.PathAuthSigninPhoneNumberAPI,
